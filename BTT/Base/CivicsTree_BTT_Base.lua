...
local tCivicsWithUniques:table = {};

function LateInitialize()
    Initialize_CivicsWithUniques();
...
function PopulateNode(uiNode, playerTechData)
    ...
    if uiNode.Name == nil then
        ...
        if uiNode.IsMarked == nil then uiNode.IsMarked = ( tCivicsWithUniques[uiNode.Type] == true ); end
...
function Initialize_CivicsWithUniques()
    --print("FUN Initialize_CivicsWithUniques");
    local localPlayerID:number = Game.GetLocalPlayer();
    ...
    for _,item in ipairs(uniqueUnits) do
        ...
        if itemInfo and itemInfo.PrereqCivic ~= nil then tCivicsWithUniques[ itemInfo.PrereqCivic ] = true; end
    end
