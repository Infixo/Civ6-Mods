print("Loading TechAndCivicSupport.lua from Real Eurekas mod");
-- ===========================================================================
...
function CanShowTrigger(iTechID:number, bCivic:boolean)
	...
	if bCivic then
		...
		return pPlayerTechs:GetResearchProgress(iTechID) > 0;
	end
end
  ...
  for row in GameInfo.Boosts() do
    if row.TechnologyType == techType then
      ...
      boostAmount = (row.Boost *.01 ) * researchCost;		--Convert the boost value to decimal and determine the actual boost amount.
			...
			if CanShowTrigger(kTech.Index, false) then triggerDesc = row.TriggerDescription;
			else triggerDesc = "LOC_REUR_QUOTE_"..math.random(22); end
			-- Infixo: end
      break;
    end
  end
  ...
  for row in GameInfo.Boosts() do
    if row.CivicType == civicType then
      ...
      boostAmount = (row.Boost *.01 ) * progressCost;		--Convert the boost value to decimal and determine the actual boost amount.
			-- Infixo: start
			--triggerDesc = row.TriggerDescription;
			if CanShowTrigger(kCivic.Index, true) then triggerDesc = row.TriggerDescription;
			else triggerDesc = "LOC_REUR_QUOTE_"..math.random(22); end
			-- Infixo: end
      break;
    end
...
print("Finished loading TechAndCivicSupport.lua from Real Eurekas mod");